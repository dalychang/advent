...
public class Puzzlev3 {
  ...
  public record DirectionKey(Direction direction, int count) {}
    ...
    for (int i = 1; i < losses.length; i++) {
      {
        ...
        DirectionKey directionKey = new DirectionKey(Direction.EAST, 1);
        bestLosses.get(directionKey)[seedPath.currentPosition.y][seedPath.currentPosition.x] = seedPath.heatLoss;
      }
      {
        DirectionKey directionKey = new DirectionKey(Direction.SOUTH, 1);
      ...
      for (Direction direction : Direction.values()) {
        if (isDirectionValid(path.lastDirections, direction)) {
          ...
          if (newPath.move(losses, direction)) {
            DirectionKey key = makeDirectionKey(newPath.lastDirections);
            if (newPath.heatLoss < bestLosses.get(key)[newPath.currentPosition.y][newPath.currentPosition.x]) {
              bestLosses.get(key)[newPath.currentPosition.y][newPath.currentPosition.x] = newPath.heatLoss;
              paths.add(newPath);
            }
            //if (iter % 100000 == 0)
  ...
  private static DirectionKey makeDirectionKey(LinkedList<Direction> lastDirections) {
    int count = 0;
    Direction firstDirection = lastDirections.getFirst();
    for (Direction direction : lastDirections) {
    ...
    return new DirectionKey(firstDirection, count);
  }
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (Direction direction : Direction.values()) {
      ...
      for (int k = 1; k <= 3; k++) {
        DirectionKey directionKey = new DirectionKey(direction, k);
        ...
        bestLosses.put(directionKey, bestLossesArray);
      }
    }
