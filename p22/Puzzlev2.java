...
public class Puzzlev2 {
  ...
  private static long simulateChainReaction(HashMultimap<Brick, Brick> brickSupportsValueMap, HashMultimap<Brick, Brick> brickSupportedByValueMap, Brick brickToCheck) {
    long answer = 0;
    LinkedList<Brick> bricksToCheck = new LinkedList<>();
    bricksToCheck.addLast(brickToCheck);
    Set<Brick> fallenBricks = new HashSet<>();
    fallenBricks.add(brickToCheck);
    
    while (!bricksToCheck.isEmpty()) {
      Brick brick = bricksToCheck.poll();
      Set<Brick> bricksSupportedByThis = brickSupportsValueMap.get(brick);
      for (Brick supportedBrick : bricksSupportedByThis) {
        Set<Brick> supportingBricks = new HashSet<>(brickSupportedByValueMap.get(supportedBrick));
        supportingBricks.removeAll(fallenBricks);
        if (supportingBricks.isEmpty()) {
          fallenBricks.add(supportedBrick);
          bricksToCheck.addLast(supportedBrick);
        }
      }
    }
  ...
  public static void main(String[] args) throws Exception {
    ...
    long answer = 0;
    for (Brick brick : bricks) {
      answer += simulateChainReaction(brickSupportsValueMap, brickSupportedByValueMap, brick);
    }
