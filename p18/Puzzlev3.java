...
public class Puzzlev3 {
  ...
  public static class GrowingGrid {
    ...
    public List<Long> getHorizontalPoints() {
      Set<Long> set = new HashSet<>();
      for (Point point : points) {
        if (point.instruction().direction().isHorizontal()) {
          set.add(point.y());
        }
      }
  ...
  public static void main(String[] args) throws Exception {
    ...
    long insideSpots = 0;
    long dugTotal = 0;
    List<Long> yPoints = grid.getHorizontalPoints();
    long previousYPoint = grid.getMinY();
    for (Long y : yPoints) {
      long previousGap = y - previousYPoint - 1;
      if (previousGap > 0) {
        insideSpots += previousGap * countSpots(grid, y - 1);
      }
      insideSpots += countSpots(grid, y);
      previousYPoint = y;
    }
  ...
  public static long countSpots(GrowingGrid grid, long y) {
    int edges = 0;
    long spots = 0;
    List<Point> points = grid.getRelevantPoints(y);
    long currentX = grid.getMinY();
    for (Point point : points) {
      if (edges == 2 && point.x() > currentX) {
        spots += (point.x() - currentX); 
      }
      if (point.instruction().direction().isVertical()) {
        if (point.y() != y && point.y() + point.instruction().direction().dy * point.instruction().meters() != y) {
          edges = (edges + 2) % 4;
          spots++; //dug + 1
        }
        ...
        // dug + n
        spots += point.instruction().meters() + 1;
