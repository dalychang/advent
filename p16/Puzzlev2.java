...
public class Puzzlev2 {
  ...
  public static long calculate(List<String> map, Beam startingBeam) {
    boolean[][] bitmap = new boolean[map.size()][map.get(0).length()];
    Set<Beam> seenBeams = new HashSet<>();
    LinkedList<Beam> beams = new LinkedList<>();
    
    beams.add(startingBeam);
    simulate(beams, bitmap, map, seenBeams);
  ...
  public static void main(String[] args) throws Exception {
    ...
    int eastMost = lines.get(0).length() - 1;
    for (int i = 0; i < lines.size(); i++) {
      beams.add(new Beam(0, i, Direction.EAST));
      beams.add(new Beam(eastMost, i, Direction.WEST));
    }
    for (int i = 0; i <= eastMost; i++) {
      beams.add(new Beam(i, 0, Direction.SOUTH));
      beams.add(new Beam(i, lines.size() - 1, Direction.NORTH));
    }
